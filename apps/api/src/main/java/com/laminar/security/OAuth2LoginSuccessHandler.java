 ...
 *
 * <p>OAuth 인증 성공 후: 이메일로 사용자 find-or-create(신규는 개인 워크스페이스 생성) → 기존 세션
 * 메커니즘({@link SessionService})으로 토큰 발급 → {@code laminar-session} 쿠키 설정 → SPA 루트로
 * 리다이렉트. 비밀번호 로그인/가입과 **동일한 세션·쿠키**를 써서 인증 경로를 통일한다.
 *
 * <p>OAuth 핸드셰이크용 HTTP 세션(JSESSIONID)은 쿠키 발급 직후 invalidate해, 이후 요청이
 * {@code laminar-session} 쿠키(SOR)로만 인증되도록 한다(OAuth2 SecurityContext 잔존 차단).
 */
@Component

...
public class PerpetualVersionController {
    ...
    public ResponseEntity<PerpetualVersionDtos.VersionResponse> commit(
        ...
        PerpetualVersionEntity committed = service.commit(
                request.perpetualNoteId(), request.cardId(),
                request.summary(), request.bodyDiffMd(), request.markCurrent());
        return ResponseEntity.ok(toResponse(committed));
    }

    @GetMapping("/perpetual-notes/{noteId}/versions")
    public ResponseEntity<List<PerpetualVersionDtos.VersionResponse>> listByNote(@PathVariable UUID noteId) {
        return ResponseEntity.ok(
                service.listByNote(noteId).stream().map(this::toResponse).toList());
    }

    @PostMapping("/perpetual-versions/{versionId}/mark-current-diff")
    public ResponseEntity<PerpetualVersionDtos.VersionResponse> markCurrentDiff(@PathVariable UUID versionId) {
    ...
    private PerpetualVersionDtos.VersionResponse toResponse(PerpetualVersionEntity v) {
        return new PerpetualVersionDtos.VersionResponse(
                v.getId(), v.getWorkspaceId(), v.getUserId(),
                v.getPerpetualNoteId(), v.getCardId(),
                v.getVersionNumber(), v.getSummary(), v.getBodyDiffMd(),
                v.isCurrentDiff(),
                v.getCommittedAt(), v.getCreatedAt());
    }
}

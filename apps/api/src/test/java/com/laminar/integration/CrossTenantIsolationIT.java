 ...
 *
 * 두 워크스페이스(A, B)를 만들고, A의 리소스를 B 컨텍스트로 접근하면 차단(빈 결과)됨을 검증한다.
 * - findById: WorkspaceContext#ownsPersonal 명시 검증 (PK 로드는 @Filter 미적용 → 이 검증이 방어선)
 * - listByBoard: WorkspaceFilterAspect가 활성화한 Hibernate @Filter로 스코프
 */
class CrossTenantIsolationIT extends IsolationIntegrationBase {
    ...
    void seed() {
        ...
        boardA = boardService.create("A Board", "a-board-" + UUID.randomUUID(), null, null, null, null).getId();
        CardEntity card = cardService.create(new CardService.CreateInput(
                boardA, "A secret", null, "TOP SECRET",

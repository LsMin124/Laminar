 ...
 *
 * MVP placeholder: 실제 SMTP/SES 통합 전까지 logger 출력 + sent_at 마킹.
 * Spec §12.3.1 v3: 도메인 미검증 시 last_error='no_domain_verified' 마킹 (후속 구현).
 */
@Component
public class EmailOutboxFlushWorker {
    ...
    public void flush() {
        ...
        for (EmailOutboxEntity email : pending) {
            ...
            try {
                ...
                emailOutboxRepo.save(email);
                log.warn("email_outbox: send failed id={} attempt={}: {}",
                        email.getId(), email.getAttemptCount(), e.getMessage());
            }
        }
    ...
    /**
     * MVP placeholder — SMTP/SES 통합 전.
     * 실제 발송 로직은 향후 외부 lib + 도메인 검증 결합.
     */

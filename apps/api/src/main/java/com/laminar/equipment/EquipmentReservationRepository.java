 ...
 *
 * overlap 쿼리는 service pre-check + DB EXCLUDE (V11 partial constraint).
 * DB constraint는 rrule IS NULL인 단일 예약만 적용 — RRULE은 인스턴스 확장 후 별도.
 */
public interface EquipmentReservationRepository extends JpaRepository<EquipmentReservationEntity, UUID> {
    ...
    List<EquipmentReservationEntity> findByReservedByAndDeletedAtIsNullOrderByStartAtDesc(UUID userId);
}

 ...
 *
 * <p>Tree depth ≤ 10 검증 (Spec §2.4.5 추정). parent_tab_id 사이클은 self-ref 차단만 (실제 사이클은 트리 구조상 발생 불가하지만
 * service에서 직속 부모 검증).
 *
 * <p>Personal-First 격리: parent_tab_id는 자기 user의 탭이어야 (parent 조회 시 @Filter 자동).
 */
@Service
public class TabService {
  ...
  public TabMemberEntity addMember(UUID tabId, UUID cardId) {
    ...
    int nextPriority =
        memberRepo
            .findFirstByIdTabIdOrderByPriorityDesc(tabId)
  ...
  public void removeMember(UUID tabId, UUID cardId) {
    ...
    tabRepo
        ...
        .filter(t -> ctx.ownsPersonal(t.getWorkspaceId(), t.getUserId()))
        .orElseThrow(() -> new IllegalArgumentException("tab not found"));
    memberRepo.findById(new TabMemberId(tabId, cardId)).ifPresent(memberRepo::delete);
  }
  ...
  public void removeGroup(UUID tabId, UUID groupId) {
    ...
    tabRepo
        ...
        .filter(t -> ctx.ownsPersonal(t.getWorkspaceId(), t.getUserId()))
        .orElseThrow(() -> new IllegalArgumentException("tab not found"));
    tabGroupRepo.findById(new TabGroupMemberId(tabId, groupId)).ifPresent(tabGroupRepo::delete);
  }
  ...
  public List<UUID> listGroupIdsInTab(UUID tabId) {
    ...
    tabRepo
        ...
        .filter(t -> ctx.ownsPersonal(t.getWorkspaceId(), t.getUserId()))
        .orElseThrow(() -> new IllegalArgumentException("tab not found"));
    return tabGroupRepo.findByIdTabId(tabId).stream().map(m -> m.getId().getGroupId()).toList();
  }
  ...
  public List<UUID> listTabIdsForCard(UUID cardId) {
    ...
    cardRepo
        ...
        .filter(c -> ctx.ownsPersonal(c.getWorkspaceId(), c.getUserId()))
        .orElseThrow(() -> new IllegalArgumentException("card not found"));
    return memberRepo.findByIdCardId(cardId).stream().map(m -> m.getId().getTabId()).toList();
  }
  ...
  private void validateParent(UUID boardId, UUID parentTabId) {
    ...
    while (cursor != null) {
      ...
      UUID next = tabRepo.findById(cursor).map(TabEntity::getParentTabId).orElse(null);
      cursor = next;
    }
  }

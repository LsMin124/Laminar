 ...
 * X-Laminar-Workspace-Id 헤더로 PERSONAL scope 진입 후 호출.
 */
@RestController
...
public class WorkspaceController {
  ...
  public ResponseEntity<WorkspaceDtos.WorkspaceResponse> create(
      Authentication authentication, @Valid @RequestBody WorkspaceDtos.CreateRequest request) {
  ...
  @GetMapping
  public ResponseEntity<List<WorkspaceDtos.WorkspaceResponse>> listMine(
      Authentication authentication) {
    LaminarPrincipal principal = requirePrincipal(authentication);
    return ResponseEntity.ok(
        workspaceService.listForUser(principal.userId()).stream().map(this::toResponse).toList());
  }
  ...
  public ResponseEntity<WorkspaceDtos.WorkspaceResponse> updateCurrent(
    ...
    WorkspaceEntity updated =
        workspaceService.updateCurrent(
            request.name(), request.defaultTimezone(), request.settings());

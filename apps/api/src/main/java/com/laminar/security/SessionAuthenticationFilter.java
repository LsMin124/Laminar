 ...
 *
 * <p>Auth.js DB session adapter 호환 — 멤버 제거 시 즉시 revoke 가능 (DB row 삭제). 만료 세션은 무시 (cleanup cron이
 * hard delete). 토큰 미설정·무효 시 anonymous (다음 필터에서 401).
 *
 * <p>SessionService (Phase 4.2)가 cookie 발급·revoke 책임 — 이 필터는 read-only.
 */
@Component
public class SessionAuthenticationFilter extends OncePerRequestFilter {
  ...
  public SessionAuthenticationFilter(
      SessionSystemRepository sessionRepo, UserSystemRepository userRepo) {
  ...
  private void authenticate(SessionEntity session) {
    ...
    LaminarPrincipal principal =
        new LaminarPrincipal(user.getId(), user.getEmail(), user.getDisplayName());

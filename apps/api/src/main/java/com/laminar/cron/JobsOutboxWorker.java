 ...
 *
 * MVP placeholder: kind별 dispatcher는 logger 출력 + complete. 실제 처리 로직은
 * RRULE expand · GCal sync · SM 동기 등 각 cron에서 별도 구현 (또는 본 worker에 dispatch table 추가).
 *
 * ShedLock으로 다중 인스턴스 동시 실행 시 1개만 처리. SKIP LOCKED는 JobsOutboxService.claimBatch
 * 내부에서 추가 안전망.
 */
@Component

 ...
 *
 * <p>정책: - 세션 IF_REQUIRED — 평시 쿠키(laminar-session) 인증이 SOR이라 API는 무세션. OAuth2 login의 인가요청(state)
 * 저장에만 HTTP 세션을 쓰고, 성공 핸들러가 즉시 invalidate. - Spring CSRF 토큰 비활성화 — 대신 CsrfHeaderFilter가
 ...
 * find-or-create + laminar-session 쿠키 발급(비밀번호 경로와 통일). - 보안 헤더: HSTS, X-Content-Type-Options,

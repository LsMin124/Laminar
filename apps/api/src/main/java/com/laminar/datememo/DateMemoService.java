 ...
 *
 * PK = (board_id, user_id, date) 복합. body_md ≤ 10KB (Spec §2.5.2).
 * upsert 패턴: 같은 (board, user, date) 키로 INSERT/UPDATE 자동.
 */
@Service
public class DateMemoService {
    ...
    @Transactional
    public DateMemoEntity upsert(UUID boardId, LocalDate date, String bodyMd, Map<String, Object> attrs) {
        ...
        DateMemoEntity memo = memoRepo.findById(id).orElseGet(() -> {
    ...
    public void delete(UUID boardId, LocalDate date) {
        WorkspaceContext ctx = requirePersonalWritable();
        memoRepo.findById(new DateMemoId(boardId, ctx.userId(), date))
                .ifPresent(memoRepo::delete);
    }

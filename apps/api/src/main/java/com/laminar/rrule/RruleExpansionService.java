 ...
 *
 * 멱등성: 마스터 카드 + start_date 키로 attrs.rrule_master_id + attrs.rrule_occurrence_date 사용.
 * 같은 (master, date) 인스턴스가 이미 있으면 skip — 매 cron 호출이 안전.
 *
 * Personal-First 격리는 호출자 책임 (cron worker가 마스터 카드 user 컨텍스트로 진입).
 */
@Service
public class RruleExpansionService {
    ...
    public int expandMaster(CardEntity master, LocalDate windowStart, LocalDate windowEnd) {
        ...
        for (CardEntity existing : cardRepo
                .findByBoardIdAndDeletedAtIsNullOrderByPriorityAsc(master.getBoardId())) {
        ...
        if (created > 0) {
            log.info("rrule expand: master={} created {} instances ({}~{})",
                    master.getId(), created, windowStart, windowEnd);
        }
        return created;
    }
    ...
    private CardEntity cloneFromMaster(CardEntity master, LocalDate occurrence) {
        ...
        if (master.getEndDate() != null && master.getStartDate() != null) {
            long span = java.time.temporal.ChronoUnit.DAYS.between(
                    master.getStartDate(), master.getEndDate());
        ...
        instance.setImportance(master.getImportance() == null ? CardImportance.NORMAL : master.getImportance());
        instance.setRrule(null);
        instance.setOrigin(CardOrigin.RRULE_EXPANSION);
        instance.setPriority(master.getPriority());

        Map<String, Object> attrs = new HashMap<>(master.getAttrs() == null ? Map.of() : master.getAttrs());

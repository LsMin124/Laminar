 ...
 *
 * 검증:
 *   - from_card_id != to_card_id (DB chk_card_relations_self 일치)
 *   - 같은 board에 속한 두 카드만 (board_id 일치 강제 — DB FK는 board_id 명시지만 application 검증)
 */
@Service
public class CardRelationService {
    ...
    public CardRelationEntity create(
        ...
        relation.setRelationKind(relationKind == null || relationKind.isBlank() ? "default" : relationKind);

 ...
 *   - INTERVAL=정수 (기본 1)
 *   - COUNT=정수 (반복 횟수)
 *   - UNTIL=YYYYMMDD or YYYYMMDDTHHMMSSZ (RFC 5545 date or datetime)
 *
 * 미지원: BYDAY, BYMONTHDAY, BYSETPOS, FREQ=HOURLY/MINUTELY/SECONDLY/YEARLY (후속).
 */
public final class RruleParser {

    private static final DateTimeFormatter UNTIL_DATE = DateTimeFormatter.BASIC_ISO_DATE;
    private static final DateTimeFormatter UNTIL_DATETIME = new DateTimeFormatterBuilder()
    ...
    private static LocalDate parseUntil(String raw) {
        try {
            return raw.length() == 8 ? LocalDate.parse(raw, UNTIL_DATE)
                    : LocalDate.parse(raw, UNTIL_DATETIME);
        } catch (Exception e) {
            throw new IllegalArgumentException("invalid UNTIL: " + raw + " (expected YYYYMMDD or YYYYMMDDTHHMMSSZ)");
        }
    }
}

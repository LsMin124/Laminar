 ...
 *
 * workspace_id nullable + @Filter 미부착 → SYSTEM scope에서만 사용.
 * SKIP LOCKED 힌트로 동시 워커가 같은 job 가져가지 않게.
 */
public interface JobsOutboxSystemRepository
        extends JpaRepository<JobsOutboxEntity, UUID>, SystemRepository {

    /**
     * 실행 가능한 job 배치 claim — SKIP LOCKED + LIMIT.
     * 호출자는 트랜잭션 안에서 lock 잡고 즉시 처리 (completed_at/failed_at set).
     */
    ...
    @Query("""
            ...
            """)
    List<JobsOutboxEntity> claimBatch(@Param("now") OffsetDateTime now, org.springframework.data.domain.Pageable pageable);

 ...
 *
 * 흐름:
 *   1. invite(): SecureRandom 32B raw token + SHA-256 hash 저장 + email_outbox 큐 INSERT
 *   2. accept(rawToken): hash 매칭 + 만료/revoked 체크 + 멤버 INSERT (사용자 미존재 시 빈 Optional)
 *   3. revoke(): revoked_at 마킹 (hard delete 아님)
 */
@Service
public class InvitationService {
    ...
    @Transactional
    public InvitationIssue invite(UUID workspaceId, String email, WorkspaceRole role, UUID invitedBy) {
    ...
    public Optional<WorkspaceMemberEntity> accept(String rawToken) {
        Optional<WorkspaceInvitationEntity> maybeInvite = invitationRepo.findByTokenHash(sha256(rawToken))
    ...
    public void revoke(UUID invitationId) {
        invitationRepo.findById(invitationId).ifPresent(invitation -> {
    ...
    private void enqueueInvitationEmail(String recipientEmail, String rawToken) {
        ...
        String workspaceName = WorkspaceContextHolder.get() == null
                ? "워크스페이스"

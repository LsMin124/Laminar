...
public class PerpetualNoteController {
    ...
    public ResponseEntity<PerpetualNoteDtos.PerpetualNoteResponse> create(
        ...
        PerpetualNoteEntity note = service.create(
                request.boardId(), request.tabId(), request.parentPerpetualId(),
                request.title(), request.bodyMd(), request.attrs());
        return ResponseEntity.ok(toResponse(note));
    }

    @GetMapping("/boards/{boardId}/perpetual-notes")
    public ResponseEntity<List<PerpetualNoteDtos.PerpetualNoteResponse>> listByBoard(@PathVariable UUID boardId) {
        return ResponseEntity.ok(
                service.listByBoard(boardId).stream().map(this::toResponse).toList());
    }

    @GetMapping("/tabs/{tabId}/perpetual-notes")
    public ResponseEntity<List<PerpetualNoteDtos.PerpetualNoteResponse>> listByTab(@PathVariable UUID tabId) {
        return ResponseEntity.ok(
                service.listByTab(tabId).stream().map(this::toResponse).toList());
    }

    @GetMapping("/tabs/{tabId}/perpetual-notes/roots")
    public ResponseEntity<List<PerpetualNoteDtos.PerpetualNoteResponse>> listRootsByTab(@PathVariable UUID tabId) {
        return ResponseEntity.ok(
                service.listRootsByTab(tabId).stream().map(this::toResponse).toList());
    }

    @GetMapping("/perpetual-notes/{noteId}/children")
    public ResponseEntity<List<PerpetualNoteDtos.PerpetualNoteResponse>> listChildren(@PathVariable UUID noteId) {
        return ResponseEntity.ok(
                service.listChildren(noteId).stream().map(this::toResponse).toList());
    }
    ...
    public ResponseEntity<PerpetualNoteDtos.PerpetualNoteResponse> update(
            ...
            @Valid @RequestBody PerpetualNoteDtos.UpdateRequest request) {
        PerpetualNoteEntity updated = service.update(
                noteId, request.title(), request.bodyMd(), request.parentPerpetualId(), request.attrs());
    ...
    private PerpetualNoteDtos.PerpetualNoteResponse toResponse(PerpetualNoteEntity n) {
        return new PerpetualNoteDtos.PerpetualNoteResponse(
                n.getId(), n.getWorkspaceId(), n.getUserId(),
                n.getBoardId(), n.getTabId(), n.getParentPerpetualId(),
                n.getTitle(), n.getBodyMd(), n.getPriority(),
                n.getAttrs(), n.getCreatedAt(), n.getUpdatedAt());
    }
}

 ...
 *
 * 향후 90일 window까지 인스턴스 카드 생성 (origin=rrule_expansion).
 * 멱등: RruleExpansionService가 attrs.rrule_master_id + occurrence_date로 중복 방지.
 *
 * SYSTEM scope로 진입 — 모든 user의 마스터 카드 처리 (Personal-First filter 미활성).
 */
@Component
public class RruleExpansionWorker {
    ...
    public void runExpansion() {
        ...
        try {
            ...
            log.info("rrule expansion: masters={}, processed={}, instances_created={}, errors={}",
                    masters.size(), processed, totalCreated, errors);
        } finally {

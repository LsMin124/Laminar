 ...
 *
 * 정책:
 *   - workspace OWNER만 호출 가능
 *   - 모든 호출은 audit_log 자동 기록 (action=admin.*, actor + target)
 *   - cross-user 메타 조회 허용, body_md 등 본문은 escape hatch 호출 + reason 명시 필요
 *
 * 구현: SYSTEM scope로 잠시 진입하여 Personal-First 필터 회피, workspace_id 일치 검증은
 * application 단에서 명시. 호출 끝에 OWNER context 복원 + audit append.
 */
@Service
public class AdminWorkspaceService {
    ...
    public List<BoardEntity> listAllBoards() {
        ...
        try {
            ...
            filterActivator.activate();
            auditService.append(owner.workspaceId(),
                    "admin.boards.list", "board", null,
    ...
    public List<Map<String, Object>> listCardMetadataByBoard(UUID boardId) {
        ...
        try {
            ...
            filterActivator.activate();
            auditService.append(owner.workspaceId(),
                    "admin.cards.list_metadata", "board", boardId,
    ...
    public Optional<CardEntity> revealCardBody(UUID cardId, String reason) {
        ...
        try {
            ...
            auditService.append(owner.workspaceId(),
                    "admin.card.reveal_body", "card", cardId,

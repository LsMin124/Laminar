 ...
 *
 * 정책:
 *   - 세션 IF_REQUIRED — 평시 쿠키(laminar-session) 인증이 SOR이라 API는 무세션. OAuth2 login의
 *     인가요청(state) 저장에만 HTTP 세션을 쓰고, 성공 핸들러가 즉시 invalidate.
 ...
 *   - OAuth2 login (Google) 활성 — AUTH_GOOGLE_ID/SECRET 설정 시. 성공 시 OAuth2LoginSuccessHandler가
 *     이메일로 사용자 find-or-create + laminar-session 쿠키 발급(비밀번호 경로와 통일).

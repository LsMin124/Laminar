 ...
 *
 * Spec §2.4.8 + v5.1:
 *   - version_number per note auto-increment (uq_perpetual_versions_note_version 일치)
 *   - is_current_diff 정확히 1건 per note (uq_perpetual_versions_current_diff partial unique)
 *   - card_id 1:1 매핑 (uq_perpetual_versions_card partial unique on card_id IS NOT NULL)
 */
@Service
public class PerpetualVersionService {
    ...
    public PerpetualVersionService(
            PerpetualVersionRepository versionRepo,
    ...
    /**
     * 버전 commit — version_number 자동 += 1, isCurrentDiff 옵션.
     * isCurrentDiff=true 요청 시 기존 currentDiff row를 false로 update + 새 row true.
     */
    @Transactional

...
public class BoardCalendarLinkService {
  ...
  @Transactional
  public BoardCalendarLinkEntity link(
      UUID boardId, String googleCalendarId, SyncDirection direction) {
    WorkspaceContext ctx = requirePersonalWritable();
    // 이미 있으면 reactivate
    Optional<BoardCalendarLinkEntity> existing =
        linkRepo.findByBoardIdAndGoogleCalendarIdAndDeletedAtIsNull(boardId, googleCalendarId);
            ...
            .filter(
                l ->
                    WorkspaceContextHolder.require()
                        .ownsPersonal(l.getWorkspaceId(), l.getUserId()))

 ...
 *
 * <p>Spec §2.4.5: tree depth ≤ 10. board/tab nullable (자유 메모).
 */
@Service
public class PerpetualNoteService {
  ...
  public List<PerpetualNoteEntity> listRootsByTab(UUID tabId) {
    WorkspaceContextHolder.requirePersonal();
    return noteRepo.findByTabIdAndParentPerpetualIdIsNullAndDeletedAtIsNullOrderByPriorityAsc(
        tabId);
  }
  ...
  public PerpetualNoteEntity update(
      UUID noteId, String title, String bodyMd, UUID parentPerpetualId, Map<String, Object> attrs) {
    WorkspaceContext ctx = requirePersonalWritable();
  ...
  private void validateParent(UUID parentPerpetualId) {
    PerpetualNoteEntity parent =
        noteRepo
            .findById(parentPerpetualId)
            .filter(n -> n.getDeletedAt() == null)
    ...
    while (cursor != null) {
      ...
      UUID next =
          noteRepo.findById(cursor).map(PerpetualNoteEntity::getParentPerpetualId).orElse(null);
      cursor = next;
    }
  }

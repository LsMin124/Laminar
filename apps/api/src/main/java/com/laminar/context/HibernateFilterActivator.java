 ...
 * 요청 필터 시점엔 Session이 없으므로, 트랜잭션 내부에서 도는 {@code WorkspaceFilterAspect}가 리포지토리 호출 직전 {@link
 * #applyForCurrentSession()}를 호출해 결선한다.
 *
 * <p>- SYSTEM / 컨텍스트 없음: 두 필터 비활성 (cron·운영 콘솔 cross-user·인증 전) - WORKSPACE_SHARED:
 * workspaceSharedFilter만 활성 - PERSONAL: 두 필터(workspace_id + user_id) 활성
 *
 * <p>주의: Hibernate @Filter는 {@code session.find()}(PK 직접 로드)에는 적용되지 않는다. 단건 UUID 접근은 도메인 서비스의 명시적
 * 소유권 검증(WorkspaceContext#ownsPersonal 등)이 최종 방어선이며, 본 필터는 derived/list 쿼리(findByBoardId 등)를
 * fail-closed로 차단한다.
 */
@Component
public class HibernateFilterActivator {
  ...
  /**
   * 활성 트랜잭션이 있을 때 현재 Session의 필터 상태를 WorkspaceContext에 맞춰 동기화. 트랜잭션이 없으면 skip — 그 경우 발생하는 단건 PK 로드는
   * 어차피 필터 대상이 아니다.
   */
  ...
  /** 기존 호출처(AdminWorkspaceService, RruleExpansionWorker) 호환 별칭 — 현재 컨텍스트 기준으로 세션 필터를 동기화한다. */

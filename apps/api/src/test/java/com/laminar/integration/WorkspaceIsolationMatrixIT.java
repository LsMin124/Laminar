 ...
 *
 * Spec §3.2 격리 정책:
 *   1. SYSTEM scope에서 personal-first read → 모든 row 보임 (필터 미활성)
 *   2. PERSONAL scope에서 자기 user board → 보임
 *   3. PERSONAL scope에서 같은 workspace 다른 user board → 0건 (cross-user 누출 0)
 *   4. PERSONAL scope에서 다른 workspace board → 0건 (cross-workspace 누출 0)
 *   5. SystemRepository (UserSystemRepository)는 모든 사용자 read (격리 우회)
 */
class WorkspaceIsolationMatrixIT extends IsolationIntegrationBase {
    ...
    void matrix_1_system_scope_sees_all_boards() {
        ...
        assertThat(all).extracting(BoardEntity::getId)
    ...
    void matrix_2_personal_scope_userA_in_wsX_sees_only_own_board() {
        ...
        assertThat(visible).extracting(BoardEntity::getId)
    ...
    void matrix_5_system_repository_bypasses_filter() {
        ...
        assertThat(allUsers).extracting(UserEntity::getId)

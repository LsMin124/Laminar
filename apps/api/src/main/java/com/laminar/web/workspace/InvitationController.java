 ...
 *
 * invite는 워크스페이스 진입 후 (PERSONAL scope) OWNER/MEMBER만 (canWrite).
 * accept는 인증 후 SYSTEM scope에서 호출 (워크스페이스 미선택 상태에서 토큰 제출).
 */
@RestController
public class InvitationController {
    ...
    public ResponseEntity<InvitationDtos.InviteResponse> invite(
            ...
            @Valid @RequestBody InvitationDtos.InviteRequest request) {
        ...
        InvitationService.InvitationIssue issue = invitationService.invite(
                workspaceId, request.email(), request.role(), principal.userId());
        return ResponseEntity.ok(new InvitationDtos.InviteResponse(
                issue.invitationId(),
                issue.rawToken(),
                request.email(),
                request.role()));
    ...
    public ResponseEntity<AcceptResponse> accept(@Valid @RequestBody InvitationDtos.AcceptRequest request) {
        return invitationService.accept(request.token())
                .map(m -> ResponseEntity.ok(new AcceptResponse(

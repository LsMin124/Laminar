 ...
 *
 * 엔드포인트:
 *   GET /boards (cross-user 메타 목록)
 *   GET /boards/{boardId}/cards (메타만, body 제외)
 *   POST /cards/{cardId}/reveal-body (escape hatch — reason 필수, audit 강한 기록)
 */
@RestController
...
public class AdminController {
    ...
    public ResponseEntity<List<BoardSummaryResponse>> listAllBoards() {
        return ResponseEntity.ok(adminService.listAllBoards().stream()
                .map(AdminController::toBoardSummary)
                .toList());
    }
    ...
    public ResponseEntity<CardBodyRevealResponse> revealCardBody(
            ...
            @Valid @RequestBody RevealBodyRequest request) {
        return adminService.revealCardBody(cardId, request.reason())
                .map(card -> new CardBodyRevealResponse(
                        card.getId(), card.getUserId(), card.getTitle(), card.getBodyMd()))
                .map(ResponseEntity::ok)
                .orElseGet(() -> ResponseEntity.notFound().build());
    }

    public record RevealBodyRequest(
    ...
    public record CardBodyRevealResponse(
            UUID cardId,
            UUID userId,
            String title,
            String bodyMd
    ...
    private static BoardSummaryResponse toBoardSummary(BoardEntity b) {
        return new BoardSummaryResponse(
                b.getId(), b.getWorkspaceId(), b.getUserId(),
                b.getName(), b.getSlug(), b.getPriority());
    }
}

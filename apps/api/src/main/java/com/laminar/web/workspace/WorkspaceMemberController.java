 ...
 *
 * 모두 워크스페이스 진입 후 호출 (X-Laminar-Workspace-Id 헤더 필수).
 * 변경 작업은 OWNER (canWrite) 만 — owner 제거 차단.
 */
@RestController
...
public class WorkspaceMemberController {
    ...
    public ResponseEntity<List<MemberResponse>> list() {
        return ResponseEntity.ok(memberService.listCurrentMembers().stream()
    ...
    public ResponseEntity<MemberResponse> updateRole(
        ...
        WorkspaceMemberEntity updated = memberService.updateRole(
                userId, request.role(), principal.userId());
    ...
    private static MemberResponse toResponse(WorkspaceMemberService.MemberView v) {
        return new MemberResponse(
                v.workspaceId(), v.userId(), v.email(), v.displayName(),
                v.role(), v.joinedAt());
    }

    private static MemberResponse toResponseFromEntity(WorkspaceMemberEntity m) {
        return new MemberResponse(
                m.getId().getWorkspaceId(), m.getId().getUserId(),
                null, null, m.getRole(), m.getJoinedAt());
    }

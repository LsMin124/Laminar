...
/**
 * 탭 Repository — Personal-First (@Filter 자동).
 * board별 priority 정렬·tree root 조회·max priority 조회가 hot path.
 */
public interface TabRepository extends JpaRepository<TabEntity, UUID> {
    ...
    List<TabEntity> findByBoardIdAndParentTabIdIsNullAndDeletedAtIsNullOrderByPriorityAsc(UUID boardId);

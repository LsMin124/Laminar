 ...
 *
 * Spec §2.10.10 + §2.10.12: text/number/enum/bool/datetime 5종 컬럼, key+label,
 * priority로 정렬. 값은 JSONB로 컬럼 key → value 저장.
 */
@Service
public class EquipmentLogService {
    ...
    /**
     * 필수 컬럼 누락 + 컬럼 type별 value 검증.
     * type=number → numeric String, bool → true/false, datetime → ISO, enum → enum_values 멤버.
     */
    private void validateValues(UUID equipmentId, Map<String, Object> values) {
        List<EquipmentLogColumnEntity> columns = columnRepo
                .findByEquipmentIdAndDeletedAtIsNullOrderByPriorityAsc(equipmentId);

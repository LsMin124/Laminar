 ...
 *
 * Spring Security FilterChain (default Order=-100) 이후 동작 (Order=50). SessionAuthenticationFilter가
 * SecurityContext에 LaminarPrincipal을 채운 다음 본 필터가 워크스페이스 진입 여부를 결정.
 *
 * scope 결정 매트릭스:
 *   - 비인증 + workspaceId 없음 → SYSTEM (health/login 등)
 *   - 인증 + workspaceId 없음 → SYSTEM (워크스페이스 미선택, /api/workspaces 등 글로벌)
 *   - 인증 + workspaceId 있고 활성 멤버 → PERSONAL (workspaceId·userId·role)
 *   - 인증 + workspaceId 있는데 멤버 아님 → 403 Forbidden (격리 위반 시도)
 *   - 비인증 + workspaceId 있음 → 401 Unauthorized
 */
@Component
@Order(50)
public class WorkspaceContextRequestFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(
        ...
        if (workspaceId == null) {
            ...
            Optional<WorkspaceMemberEntity> member = memberRepo
                    .findByIdWorkspaceIdAndIdUserIdAndRemovedAtIsNull(workspaceId, userId);
            if (member.isEmpty()) {

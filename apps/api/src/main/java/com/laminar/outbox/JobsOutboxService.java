 ...
 *
 * Spec §3.7: enqueue는 비즈니스 트랜잭션 안에서, polling은 별도 cron 워커.
 * SKIP LOCKED로 다중 워커 동시 실행 시 같은 job 중복 처리 차단.
 *
 * 시스템 컨텍스트만 호출 — workspace 무관 또는 명시 workspaceId.
 */
@Service
public class JobsOutboxService {
    ...
    /**
     * 실패 처리 + 재시도 backoff (attempt^2 분 후 run_after).
     * MAX_ATTEMPTS 초과 시 failed_at 영구 마킹.
     */
    @Transactional

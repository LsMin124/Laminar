...
public class TabController {
    ...
    @PostMapping("/tabs")
    public ResponseEntity<TabDtos.TabResponse> create(@Valid @RequestBody TabDtos.CreateRequest request) {
        TabEntity tab = tabService.create(
                request.boardId(), request.parentTabId(), request.name(),
                request.visible(), request.collapsed(), request.showLabel(),
                request.labelColor(), request.attrs());
    ...
    public ResponseEntity<TabDtos.TabResponse> update(
            ...
            @Valid @RequestBody TabDtos.UpdateRequest request) {
        TabEntity updated = tabService.update(
                tabId, request.name(), request.parentTabId(),
                request.visible(), request.collapsed(), request.showLabel(),
                request.labelColor(), request.attrs());
    ...
    private TabDtos.TabResponse toResponse(TabEntity t) {
        return new TabDtos.TabResponse(
                ...
                t.getParentTabId(), t.getName(), t.getPriority(),
                t.isVisible(), t.isCollapsed(), t.isShowLabel(), t.getLabelColor(),
                t.getAttrs(), t.getCreatedAt(), t.getUpdatedAt());
    }
}
